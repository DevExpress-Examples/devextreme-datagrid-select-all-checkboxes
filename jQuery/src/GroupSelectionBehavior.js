class GroupSelectionBehavior {
  ...
  groupTemplate(cellElement, cellData) {
    ...
    const that = this;
    const checkBox = $('<div>').attr('id', checkBoxId).addClass('group-selection-check-box').appendTo(flex)
      .dxCheckBox({
        visible: false,
        onValueChanged(valueArgs) {
          if (that.skipCheckBoxValueHandling) return;
          const childRowKeys = valueArgs.component.option('childRowKeys');
          if (valueArgs.value) cellData.component.selectRows(childRowKeys, true);
          else cellData.component.deselectRows(childRowKeys);
        },
      })
      .dxCheckBox('instance');
    ...
    this.getSelectedKeys(cellData.component).then((selectedKeys) => {
      if (this.cacheGroupRequests && this.rowKeysCache[checkBoxId]) {
        ...
        const groupedColumns = cellData.component.getVisibleColumns()
  ...
  repaintGroupRowTree(grid, groupRows) {
    const topGroupRow = groupRows
      .filter((r) => r.isExpanded)
      .reduce(
        (acc, current) => ((!acc || acc.key.length > current.key.length) ? current : acc),
        null,
      );
    if (topGroupRow) {
